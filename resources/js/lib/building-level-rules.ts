...
): BuildingUnlockRule | undefined {
    return unlockRules.find(
        (rule) =>
            rule.building_type_id === buildingTypeId &&
            rule.th_level === thLevel,
    );
}

/** Highest placeable level, or 0 when the building is not unlocked at this Town Hall. */
export function maxLevelFor(
    buildingTypeId: number,
    thLevel: number,
    unlockRules: BuildingUnlockRule[],
): number {
    return (
        findRule(buildingTypeId, thLevel, unlockRules)?.max_building_level ?? 0
    );
}
