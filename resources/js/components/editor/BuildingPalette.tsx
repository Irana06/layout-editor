...
export function BuildingPalette({ buildingTypes, unlockRules }: Props) {
    ...
    const placedCounts = useMemo(() => {
        ...
        for (const placement of state.placements) {
            counts.set(
                ...
                (counts.get(placement.buildingTypeId) ?? 0) + 1,
            );
        }
    ...
    const arm = (type: BuildingType, level: BuildingLevel) => {
        ...
        dispatch({
            type: 'SET_STATUS',
            status: `Mode tempatkan: ${type.name} (Lv${level.level}).`,
        });
    };
    ...
    const handleClick = (type: BuildingType, maxLevel: number) => {
        ...
        const preferred =
            unlocked.find((l) => l.level === lastPicked) ??
    ...
    return (
        <div>
            ...
            />
            {!anyUnlocked && (
                <div className="mb-3 rounded-lg border border-dashed border-border/60 bg-muted/30 p-3 text-xs leading-relaxed">
                    <p className="mb-1 font-medium">
                        Belum ada bangunan yang terbuka di TH{state.thLevel}.
                    </p>
                    ...
                    <div key={category}>
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                            {types.map((type) => {
                                ...
                                const maxCount = maxCountFor(
                                    type.id,
                                    state.thLevel,
                                    unlockRules,
                                );
                                const unlocked = type.levels.filter(
                                    (l) => l.level <= maxLevel,
                                );
                                const placedCount =
                                    placedCounts.get(type.id) ?? 0;
                                const atLimit =
                                    maxCount !== null &&
                                    placedCount >= maxCount;
                                const isArmed =
                                    state.armed?.buildingTypeId === type.id;
                                const thumbLevel =
                                    unlocked[unlocked.length - 1] ??
                                    type.levels[0];
                                ...
                                if (atLimit) {
                                    return (
                                        <Tooltip key={type.id}>
                                            ...
                                            <TooltipContent>
                                                Sudah mencapai batas {maxCount}{' '}
                                                bangunan di TH{state.thLevel}
                                            </TooltipContent>
                                        </Tooltip>
                                    );
                                }
