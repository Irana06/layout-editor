...
type EditorContextValue = {
    ...
    zoomActionsRef: MutableRefObject<{ setZoomPercent: (percent: number) => void } | null>;
    imageCache: MutableRefObject<Map<string, HTMLImageElement>>;
};

const EditorContext = createContext<EditorContextValue | null>(null);

export function EditorProvider({ children, initial }: { children: ReactNode; initial?: Partial<EditorState> }) {
    ...
    const zoomActionsRef = useRef<{ setZoomPercent: (percent: number) => void } | null>(null);
    const imageCache = useRef(new Map<string, HTMLImageElement>());
    const [zoomDisplay, setZoomDisplay] = useState(100);

    return (
        <EditorContext.Provider value={{ state, dispatch, cameraRef, baseFitRef, zoomActionsRef, imageCache, zoomDisplay, setZoomDisplay }}>
            {children}
